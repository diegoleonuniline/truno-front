...
(function() {
  ...
  const render = {
    ...
    ventas() {
      ...
      if (!ventas.length) { elements.tableContainer.style.display = 'none'; elements.mobileCards.innerHTML = ''; elements.emptyState.style.display = 'block'; elements.pagination.style.display = 'none'; return; }
      ...
      elements.tableBody.innerHTML = ventas.map(v => {
        ...
        const sl = { pendiente: 'Pendiente', parcial: 'Parcial', pagado: 'Cobrada', vencido: 'Vencida', cancelado: 'Cancelada' }[sc] || sc;
        return `<tr data-id="${v.id}">
          <td><div class="cell-main">${v.nombre_contacto || v.descripcion || 'Sin cliente'}</div><div class="cell-sub">${v.folio ? `#${v.folio}` : ''}</div></td>
          <td>${utils.formatDate(v.fecha)}</td><td>${v.folio || '-'}</td><td>${v.fecha_vencimiento ? utils.formatDate(v.fecha_vencimiento) : '-'}</td>
          <td><span class="status-badge ${sc}">${sl}</span></td>
          <td style="text-align:right;"><div class="cell-amount income">${utils.formatMoney(v.total, v.moneda)}</div>${pend > 0 && pend < parseFloat(v.total) ? `<div class="cell-sub">Pend: ${utils.formatMoney(pend)}</div>` : ''}</td>
          <td><div class="table-actions">
            ${pend > 0 ? `<button class="action-btn" title="Cobrar" data-action="collect" data-id="${v.id}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg></button>` : ''}
            <button class="action-btn" title="Editar" data-action="edit" data-id="${v.id}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
            <button class="action-btn danger" title="Eliminar" data-action="delete" data-id="${v.id}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
          </div></td></tr>`;
      }).join('');
      ...
      elements.mobileCards.innerHTML = ventas.map(v => {
        ...
        return `<div class="mobile-card" data-id="${v.id}">
          <div class="mobile-card-header"><div class="mobile-card-title">${v.nombre_contacto || v.descripcion || 'Sin cliente'}</div><div class="mobile-card-amount">${utils.formatMoney(v.total, v.moneda)}</div></div>
          <div class="mobile-card-meta"><span>${utils.formatDate(v.fecha)}</span><span>${v.folio || ''}</span></div>
          <div class="mobile-card-footer"><span class="status-badge ${sc}">${sl}</span>
            <div class="table-actions">
              ${pend > 0 ? `<button class="action-btn" data-action="collect" data-id="${v.id}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg></button>` : ''}
              <button class="action-btn" data-action="edit" data-id="${v.id}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
              <button class="action-btn danger" data-action="delete" data-id="${v.id}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
            </div>
          </div></div>`;
      }).join('');
    ...
    contactos() { elements.contactoId.innerHTML = '<option value="">-- Sin cliente --</option>' + state.contactos.map(c => `<option value="${c.id}">${c.nombre}</option>`).join(''); },
    cuentas() { elements.cobroCuenta.innerHTML = '<option value="">-- Sin registrar en banco --</option>' + state.cuentas.map(c => `<option value="${c.id}">${c.nombre} (${utils.formatMoney(c.saldo_actual)})</option>`).join(''); }
  };
  ...
  const handlers = {
    async loadData() {
      try {
        ...
        render.ventas(); render.stats(); render.contactos(); render.cuentas();
    ...
    openCreateModal() {
      state.editingId = null; elements.modalTitle.textContent = 'Nueva Venta'; elements.ventaForm.reset();
      elements.fecha.value = utils.today(); elements.ventaModal.classList.add('active'); elements.contactoId.focus();
    },
    openEditModal(v) {
      state.editingId = v.id; elements.modalTitle.textContent = 'Editar Venta';
      elements.contactoId.value = v.contacto_id || ''; elements.folio.value = v.folio || '';
      elements.fecha.value = utils.formatDateInput(v.fecha); elements.fechaVencimiento.value = utils.formatDateInput(v.fecha_vencimiento);
      elements.descripcion.value = v.descripcion || ''; elements.subtotal.value = v.subtotal || '';
      elements.impuesto.value = v.impuesto || ''; elements.total.value = v.total || '';
      elements.moneda.value = v.moneda || 'MXN'; elements.tipoCambio.value = v.tipo_cambio || 1;
      elements.uuidCfdi.value = v.uuid_cfdi || ''; elements.folioCfdi.value = v.folio_cfdi || '';
      elements.notas.value = v.notas || ''; elements.ventaModal.classList.add('active');
    },
    closeVentaModal() { elements.ventaModal.classList.remove('active'); elements.ventaForm.reset(); state.editingId = null; },
    async submitVenta(e) {
      e.preventDefault();
      const d = {
        contacto_id: elements.contactoId.value || null, folio: elements.folio.value.trim() || null,
        fecha: elements.fecha.value, fecha_vencimiento: elements.fechaVencimiento.value || null,
        ...
        impuesto: parseFloat(elements.impuesto.value) || 0, total: parseFloat(elements.total.value),
        moneda: elements.moneda.value, tipo_cambio: parseFloat(elements.tipoCambio.value) || 1,
        uuid_cfdi: elements.uuidCfdi.value.trim() || null, folio_cfdi: elements.folioCfdi.value.trim() || null,
        notas: elements.notas.value.trim() || null
      };
      elements.submitModal.classList.add('loading'); elements.submitModal.disabled = true;
      try {
        if (state.editingId) await api.updateVenta(state.editingId, d); else await api.createVenta(d);
        this.closeVentaModal(); await this.loadData();
    ...
    openCobroModal(v) {
      ...
      elements.cobroTotal.value = utils.formatMoney(v.total); elements.cobroPendiente.value = utils.formatMoney(pend);
      elements.cobroMonto.value = pend.toFixed(2); elements.cobroMonto.max = pend;
      elements.cobroFecha.value = utils.today(); elements.cobroCuenta.value = ''; elements.cobroMetodo.value = 'transferencia';
      elements.cobroModal.classList.add('active'); elements.cobroMonto.focus();
    },
    closeCobroModal() { elements.cobroModal.classList.remove('active'); elements.cobroForm.reset(); state.collectingVenta = null; },
    async submitCobro(e) {
      e.preventDefault();
      const d = { tipo: 'venta', referencia_id: state.collectingVenta.id, monto: parseFloat(elements.cobroMonto.value), fecha: elements.cobroFecha.value, metodo_pago: elements.cobroMetodo.value, cuenta_bancaria_id: elements.cobroCuenta.value || null };
      elements.submitCobro.classList.add('loading'); elements.submitCobro.disabled = true;
      try { await api.registrarCobro(d); this.closeCobroModal(); await this.loadData(); }
      catch (e) { alert(e.message); }
      finally { elements.submitCobro.classList.remove('loading'); elements.submitCobro.disabled = false; }
    },
    openDeleteModal(v) { state.deletingId = v.id; elements.deleteVentaName.textContent = v.nombre_contacto || v.folio || `Venta del ${utils.formatDate(v.fecha)}`; elements.deleteModal.classList.add('active'); },
    ...
    async confirmDelete() {
      ...
      try { await api.deleteVenta(state.deletingId); this.closeDeleteModal(); await this.loadData(); }
      catch (e) { alert(e.message); }
      finally { elements.confirmDelete.classList.remove('loading'); elements.confirmDelete.disabled = false; }
    },
    applyFilters() { state.filters.buscar = elements.searchInput.value.trim(); state.filters.estatus = elements.filterStatus.value; state.paginacion.pagina = 1; this.loadData(); },
